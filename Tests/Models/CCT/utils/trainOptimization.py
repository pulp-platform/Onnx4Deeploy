...
def convert_layernorm_to_groupnorm(input_onnx, output_onnx, num_groups=1, split_grad_nodes=False):
    """
    1. 将 LayerNorm 转换为 GroupNorm
    2. 如果开启 split_grad_nodes，则执行无 Slice 的 Stat 直连模式
    """
    model = onnx.load(input_onnx)
    graph = model.graph
                ...
                if len(node.input) > i:
                    ...
                    for init in graph.initializer:
                        if init.name == param_name:
                            array = numpy_helper.to_array(init)
                            layernorm_params[param_name] = {'num_channels': array.shape[0], 'shape': array.shape}
    ...
    for init in graph.initializer:
        if init.name in layernorm_params and len(layernorm_params[init.name]['shape']) > 1:
            array = numpy_helper.to_array(init)
            new_array = array.reshape(layernorm_params[init.name]['num_channels'], -1).mean(axis=1).astype(array.dtype)
            new_init = numpy_helper.from_array(new_array, init.name)
            init.CopyFrom(new_init)

    onnx.save(model, output_onnx)

def split_gn_to_single_stat_array(onnx_path, save_path):
    model = onnx.load(onnx_path)
    graph = model.graph
    
    # 辅助函数：从 graph 获取张量的 Shape
    def get_shape(tensor_name):
        # 检查 graph input
    ...
    for node in list(graph.node):
        if node.op_type == "GroupNormalization":
            ...
            # 推导并记录 stat 张量的 Shape [N, G, 2]
            x_shape = get_shape(X)
            if x_shape and len(x_shape) > 0:
                N = x_shape[0]
                stat_shape = [N, num_groups, 2]
                stat_vi = helper.make_tensor_value_info(stat_array_name, TensorProto.FLOAT, stat_shape)
                new_value_infos.append(stat_vi)

            # 创建拆分后的前向节点
            nodes_to_add.append(helper.make_node(
                'GroupNormStats', [X], [stat_array_name], 
                name=f"{node.name}_stats", num_groups=num_groups, epsilon=epsilon
            ))
            nodes_to_add.append(helper.make_node(
                'GroupNormForward', [X, gamma, beta, stat_array_name], [Y], 
                name=f"{node.name}_fwd", num_groups=num_groups
            ))
            nodes_to_remove.append(node)
    ...
    for node in list(graph.node):
        if node.op_type == "GroupNormalizationGrad":
            ...
            if dX != "" and dX in used_tensors:
                ...
                # 推导 grad_stat 的形状 [N, num_groups, 2]
                x_shape = get_shape(X)
                if x_shape and len(x_shape) > 0:
                    N = x_shape[0]
                    grad_stat_shape = [N, num_groups, 2]
                    grad_stat_vi = helper.make_tensor_value_info(grad_stat_name, TensorProto.FLOAT, grad_stat_shape)
                    new_value_infos.append(grad_stat_vi)

                # 第一步：计算 grad_stat (mean_gamma_dY, mean_gamma_dY_Xnorm)
                nodes_to_add.append(helper.make_node(
                    'GroupNormGradXStat', [dY, X, gamma, target_stat], [grad_stat_name],
                    name=f"{node.name}_dX_stat", num_groups=num_groups
                ))

                # 第二步：使用 grad_stat 计算 dX (支持 HW tiling)
                nodes_to_add.append(helper.make_node(
                    'GroupNormGradX', [dY, X, gamma, target_stat, grad_stat_name], [dX],
                    name=f"{node.name}_dX", num_groups=num_groups
                ))

            if dGamma != "" and dGamma in used_tensors:
                nodes_to_add.append(helper.make_node(
                    'GroupNormGradW', [dY, X, target_stat], [dGamma],
                    name=f"{node.name}_dW", num_groups=num_groups
                ))
                # 修正 dGamma 的形状：从 LayerNorm 的 [C, H, W] 改为 GroupNorm 的 [C]
                x_shape = get_shape(X)
            ...
            if dBeta != "" and dBeta in used_tensors:
                nodes_to_add.append(helper.make_node(
                    'GroupNormGradB', [dY], [dBeta],
                    name=f"{node.name}_dB"
                ))
                # 修正 dBeta 的形状：从 LayerNorm 的 [C, H, W] 改为 GroupNorm 的 [C]
                x_shape = get_shape(X)
                if x_shape and len(x_shape) >= 2:
                    C = x_shape[1]
                    # 删除旧的 value_info
    ...
    for out in graph.output:
        if '_updated' in out.name:
            ...
            if param_name in norm_param_shapes:
                ...
                print(f"✅ 修正 SGD 输出形状: {out.name} -> {correct_shape}")
