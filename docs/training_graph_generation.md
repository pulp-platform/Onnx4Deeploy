...
| `--data-size N` | `None` | MNIST pool size for epoch-cycling mode. `None`: each batch draws a fresh unique image (no loss descent). `N`: fix a pool of N images and repeat with per-epoch shuffle → loss descends visibly. |
...
| `data_size` | `int\|None` | `None` | MNIST pool size for epoch-cycling mode (see [Data Source](#data-source)) |
...
flag.  Both implementations use a **local** `numpy.random.RandomState` and do
**not** modify the global numpy seed.

### `"random"` (default)

Generates Gaussian random inputs and uniform-random integer labels.
Seed is fixed to `42` for reproducibility.  No external files required.

With random data there is no class structure, so **no loss descent is expected**
regardless of the number of batches.  This mode is intended for pure
correctness testing (device output matches Python reference).
...
### Why loss may not decrease with MNIST (and how to fix it)

Even with real MNIST images, **loss will not visibly decrease** when each
mini-batch draws a fresh, never-seen-before image.  The root cause:

```
n_batches=8, batch_size=1, data_size=None:
  batch 0 → image #31052   SGD step 0  loss ≈ 2.30
  batch 1 → image #7841    SGD step 1  loss ≈ 2.27  ← different image
  batch 2 → image #19203   SGD step 2  loss ≈ 2.35  ← different image
  ...
  Each step trains on a unique image. Weights barely move. Loss noisy.
```

SGD needs to see the **same image multiple times** to accumulate a consistent
gradient signal and reduce loss.

### Epoch cycling mode (`--data-size N`)

Set `--data-size N` to fix a pool of `N` images drawn once from MNIST.
Mini-batches are generated by repeatedly cycling through this pool with a fresh
shuffle at the start of each epoch:

```
data_size=5, n_batches=20, batch_size=1  →  4 epochs of 5 images each

Epoch 1: [img3, img1, img4, img0, img2]   batches 0..4
Epoch 2: [img2, img4, img0, img3, img1]   batches 5..9   (reshuffled)
Epoch 3: [img1, img0, img3, img4, img2]   batches 10..14
Epoch 4: [img4, img3, img1, img2, img0]   batches 15..19
```

Because weights are updated toward the same small set of images repeatedly,
**loss decreases monotonically** (the network overfits the pool).

**CLI flag:** `--data-size N`

**Rule of thumb for visible convergence:**

| data_size | Recommended n_batches | Notes |
|-----------|----------------------|-------|
| 10        | 40–100               | Very fast overfitting |
| 50        | 100–300              | Clear descent curve, still fast |
| 200       | 400–1000             | Slower but more realistic |

**Example:**

```bash
python Onnx4Deeploy.py --model SimpleMLP --mode train \
    --dataset mnist --data-size 50 --n-batches 200
```

Expected `outputs.npz["loss"]` output: steadily decreasing from ≈2.3 toward
≈0.x over 200 batches.
...
### MNIST data — 28×28 input, real images (correctness test)
...
Input size auto-adjusts to 784 (28×28).  Each batch draws a fresh image —
loss will be noisy (not descending).  This is fine for device correctness
testing.

### MNIST with epoch cycling — observe loss descent

```bash
# Pool of 50 images, cycled over 200 batches (≈4 epochs)
python Onnx4Deeploy.py --model SimpleMLP --mode train \
    --dataset mnist --data-size 50 --n-batches 200
```

Loss in `outputs.npz["loss"]` will show a clear downward trend.

For a more capable network (larger hidden layer):

```bash
python Onnx4Deeploy.py --model SimpleMLP --mode train \
    --dataset mnist --data-size 50 --n-batches 200 --hidden-size 128
```
...
```bash
python Onnx4Deeploy.py --model SimpleMLP --mode train \
    --dataset mnist --data-path /data/mnist --data-size 50 --n-batches 200
```

### MNIST with gradient accumulation + epoch cycling

```bash
python Onnx4Deeploy.py --model SimpleMLP --mode train \
    --dataset mnist --data-size 30 --n-batches 60 --n-accum 3
```
