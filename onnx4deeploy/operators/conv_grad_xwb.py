...
"""ConvGrad operator test: combined dX + dW + dB (with-bias, 3-output variant).

ORT com.microsoft.ConvGrad with-bias signature:
  Inputs : [dY (output_grad), X (input_data), W (weight), B (bias)]
  Outputs: [dX (input_grad), dW (weight_grad), dB (bias_grad)]

dB[c_out] = sum(dY[:, c_out, :, :])  over N, H, W
"""
...
class ConvGradXWBOperatorTest(BaseOperatorTest):
    """Test generator for ConvGrad (dX + dW + dB, with bias)."""
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        cfg = config.get("convgradxwb", {})
        self.output_grad_shape = tuple(cfg.get("output_grad_shape", [1, 4, 4, 4]))
        ...
        self.pads = cfg.get("pads", [1, 1, 1, 1])
    ...
    @property
    def c_out(self):
        return self.output_grad_shape[1]
    ...
    def create_onnx_graph(self, inputs: Dict[str, np.ndarray]):
        ...
        bias = np.random.randn(*self.bias_shape).astype(np.float32)

        # Graph inputs (variable)
        ...
        bias_grad_t = helper.make_tensor_value_info(
            "bias_grad", TensorProto.FLOAT, list(self.bias_shape)
        )
        ...
        bias_init = helper.make_tensor(
            name="bias",
            data_type=TensorProto.FLOAT,
            dims=self.bias_shape,
            vals=bias.flatten().tolist(),
        )

        node = helper.make_node(
            "ConvGrad",
            inputs=["output_grad", "input_data", "weight", "bias"],
        ...
        graph = helper.make_graph(
            [node],
            "convgrad_graph",
            [output_grad_t, input_data_t],
            [input_grad_t, weight_grad_t, bias_grad_t],
    ...
    def create_model(self, graph, opset_version: int = 13):
        model = helper.make_model(
            graph,
            producer_name="convgrad_xwb_test",
            opset_imports=[helper.make_opsetid("", opset_version)],
        )
        shapes = [self.input_grad_shape, self.weight_grad_shape, self.bias_shape]
    ...
    def compute_expected_output(self, inputs: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        """Compute dX, dW, and dB numerically."""
