...
def remove_identity_reducesum(input_model_path, output_model_path):
                ...
                if input_name in replacement_map:
                    ...
                    if node.op_type == "Reshape" and i == 0:  # If this is the data input to Reshape
                        ...
                        if len(node.input) > 1:  # Has shape input
                            ...
                            if not shape_exists:
                                print(
                                    f"Warning: Reshape node {node.name} has missing shape input after Identity removal"
                                )
                                ...
                                if node.output[0] in shape_info:
                                    ...
                                    print(
                                        f"  Fixed: Added shape tensor {shape_tensor_name} with shape {output_shape}"
                                    )
    ...
    new_nodes = [
        node for node in graph.node if node not in identity_nodes and node not in reducesum_nodes
    ]
...
def convert_sum_to_add(input_model_path, output_model_path):
    ...
    # <Sum_node>_intermediate_<j>" — surfaces whenever a gradient feed-point
    # has more than 3 contributors (e.g. LoRA adapters add 3 extra branches
    # per Q/K/V on the attention input).
    shape_lookup: dict = {}
    ...
    for i, node in enumerate(model.graph.node):
        ...
        if node.op_type == "Sum":
            ...
            if input_count == 1:
                ...
                # Adds are element-wise: every intermediate has the same
                # dtype/shape as the inputs we know about.
                inferred_dtype = None
                inferred_shape = None
                for inp in node.input:
...
def convert_frozen_initializers_to_constants(
    input_model_path: str,
    output_model_path: str,
    infer_model_path: str,
) -> None:
    """
    Convert frozen (non-trainable) parameters into inline ONNX Constant nodes.

    Background
    ----------
    After ``generate_artifacts()``, ORT promotes **all** model parameters
    (both trainable and frozen) to ``graph.input`` so that the runtime can
    supply their values.  The only structural difference between a trainable
    and a frozen parameter in the training model is:

    * Trainable param ``p``:  a matching ``p_grad.accumulation.buffer`` input
      exists in the graph (ORT uses it to accumulate the gradient).
    * Frozen param ``p``:     **no** such buffer exists.

    We obtain the tensor values for frozen params from ``infer_model_path``
    (the inference ONNX produced right before ``generate_artifacts``), where
    they are stored as ordinary initializers.

    Strategy
    --------
    For every frozen param ``p``:
    1. Read its tensor value from the inference model's initializers.
    2. Create a ``Constant`` node whose output name equals ``p`` (so all
       downstream consumers continue to reference the same name without
       any graph rewiring).
    3. Remove ``p`` from ``graph.input`` (it is no longer a runtime input).

    Args:
        input_model_path:  Path to the input ONNX training model
                           (produced by ``generate_artifacts``).
        output_model_path: Path where the transformed model is written.
        infer_model_path:  Path to the inference ONNX model whose
                           ``graph.initializer`` entries hold the frozen
                           parameter values.
    """
    import copy
    ...
    # ------------------------------------------------------------------ #
    # Step 1: identify trainable params via their grad-accumulation buffer #
    # ------------------------------------------------------------------ #
    # ORT names each buffer "<param_name>_grad.accumulation.buffer".
    BUFFER_SUFFIX = "_grad.accumulation.buffer"
    trainable_param_names = {
        inp.name[: -len(BUFFER_SUFFIX)] for inp in graph.input if inp.name.endswith(BUFFER_SUFFIX)
    }

    # ------------------------------------------------------------------ #
    # Step 2: load frozen param values from the inference model            #
    # ------------------------------------------------------------------ #
    infer_model = onnx.load(infer_model_path)
    infer_init_map = {init.name: init for init in infer_model.graph.initializer}

    # Frozen params = params that exist in the inference model but are NOT trainable.
    frozen_param_names = {name for name in infer_init_map if name not in trainable_param_names}

    if not frozen_param_names:
        print(
            "   convert_frozen_initializers_to_constants: nothing to do (all params are trainable)."
        )
        onnx.save(model, output_model_path)
        return

    # Keep only names that actually appear as graph inputs in the training model
    # (sanity check – they should all be there after generate_artifacts).
    graph_input_names = {inp.name for inp in graph.input}
    frozen_param_names = frozen_param_names & graph_input_names

    if not frozen_param_names:
        print(
            "   convert_frozen_initializers_to_constants: frozen params not found in graph.input."
        )
        onnx.save(model, output_model_path)
        return

    print(f"   Frozen params detected: {sorted(frozen_param_names)}")

    # ------------------------------------------------------------------ #
    # Step 3: build one Constant node per frozen param                     #
    # ------------------------------------------------------------------ #
    constant_nodes = []
    for name in sorted(frozen_param_names):
        tensor = copy.deepcopy(infer_init_map[name])
        # Constant nodes require the embedded tensor to have an empty name
        # (the node's output carries the name).
        tensor.name = ""

        const_node = onnx.NodeProto()
        const_node.op_type = "Constant"
        const_node.name = f"Constant_{name}"
        const_node.output.append(name)
    ...
    # ------------------------------------------------------------------ #
    # Step 4: prepend Constant nodes; remove frozen params from graph.input #
    # ------------------------------------------------------------------ #
    ...
    remaining_inputs = [inp for inp in graph.input if inp.name not in frozen_param_names]
    del graph.input[:]
