...
class TinyTransformerExporter(BaseONNXExporter):
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "training_strategy": "full",  # "full" | "head" | "custom" | "lora" — see get_trainable_params()
            "custom_trainable_params": [],
            # LoRA fine-tuning — when use_lora=True, A/B adapters are attached to
            # the q/k/v/out_proj projections in the attention block. Pair this with
            # training_strategy="lora" to freeze every parameter except the
            # lora_* adapters; the backward graph generated by ORT then only
            # produces gradients for those adapters.
            "use_lora": False,
            "lora_r": 4,
            "lora_alpha": 16,
            # Training loop
    ...
    def get_trainable_params(self, all_param_names: List[str]) -> List[str]:
        ...
        Strategies:
            ...
            "lora"   — train only the LoRA A/B adapters (requires use_lora=True);
                       all base weights are frozen, the backward graph contains
                       only the LoRA gradient subgraph.
        """
        ...
        if strategy == "lora" and not self.config.get("use_lora", False):
            raise ValueError(
                "training_strategy='lora' requires use_lora=True so that the model "
                "actually contains lora_* parameters."
            )
