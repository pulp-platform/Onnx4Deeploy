"""GroupNorm operator test implementation."""
...
class GroupNormOperatorTest(BaseOperatorTest):
    """Test generator for ONNX GroupNormalization operator (custom op)."""
    ...
    def load_config(self) -> Dict[str, Any]:
        """Load GroupNorm-specific configuration."""
        config = super().load_config()

        gn_config = config.get("groupnorm", {})
        self.input_shape = tuple(gn_config.get("input_shape", [1, 8, 1, 64]))
    ...
    def create_onnx_graph(self, inputs: Dict[str, np.ndarray]):
        """Create ONNX graph for GroupNormalization operator."""
        N, C, H, W = self.input_shape
        ...
        # GroupNormalizationStat node (computes mean and inv_std)
        ...
        # GroupNormalization node (applies normalization)
    ...
    def create_model(self, graph, opset_version: int = 13):
        """Create ONNX model for GroupNormalization."""
