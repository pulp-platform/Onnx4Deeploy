...
def read_csv_seb(subject):
    x = pd.read_csv(f'seb_data/X_samples_{subject}.csv')
    y = pd.read_csv(f'seb_data/y_samples_{subject}.csv')
    x = x.drop(columns=x.columns[-1])

    X_all = x.to_numpy()
    y_all = y.to_numpy()
    labels_to_keep = np.array([50, 51, 52, 53, 61, 62, 63, 64, 70, 71, 80])
    # Create a boolean mask where True indicates the label should be kept
    mask = np.isin(y_all, labels_to_keep)

    # Apply the mask to your array to filter out unwanted labels
    y = y_all[mask]
    X = X_all[mask.flatten()]

    X_CNN = np.reshape(X, (len(y), 2, 1, -1))

    # Change the labels such they are 0 to 10, they are currently [50, 51, 52, 53, 61, 62, 63, 64, 70, 71, 80]
...
def main():
    ...
    parser.add_argument('--subject', type=int, default=0, help='Subject index')
    ...
    # Initialize a list to collect data entries
    data_entries = []
    remove_length = 1.7*500
    original_length = 1000  # Assuming the original fourth dimension length
    step_size = 50  # Assuming each step reduces the tensor by 50 in the last dimension
    patient_to_do = patients[subject_index]
    print(f"Patient: {patient_to_do}")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    data_all_included, label_all_included = read_csv_seb(patient_to_do)
    X_tensor = torch.tensor(data_all_included, dtype=torch.float32)
    y_tensor = torch.tensor(label_all_included, dtype=torch.long)
    X_tensor = X_tensor.permute(0, 2, 1, 3)
    for i in range(int(remove_length / step_size) + 1):
        X_tensor_train = X_tensor[:, :, :, :original_length - i * step_size]
        ...
        data_entries.append({'Patient': patient_to_do, 'Best Accuracy': max_acc, 'Best Epoch': best_epoch, 'Method': method, 'Notch Filter': apply_notch_filter, 'Bandpass Filter': apply_bandpass_filter, 'Remove Length': removal})

    # Create a DataFrame from the collected data entries
    results_df = pd.DataFrame(data_entries)
    # Save the results to CSV file
    results_df.to_csv(f'results/{patient_to_do}_seb.csv', index=False)
